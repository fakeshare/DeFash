...
class InputPolicy(object):
    ...
    def start(self, input_manager): # 这个函数是关键函数
        """
        ...
        while input_manager.enabled and count < input_manager.event_count:
            try:
                ...
                if count == 0 and self.master is None:
                    ...
                    # 打开frida 通过packagename hook
                    shell = "python hook.py " + self.app.get_package_name()
                    child = subprocess.Popen(shell)  # frida开始hook
                    print("start hook!! shell:{}".format(shell))
                    event = self.generate_event()
                else:
                    event = self.generate_event()
                if event!="continue_lsy":
                    input_manager.add_event(event)  # 这里相当于执行
                if END_BOT == True:
                    # 关闭frida
                    if child != None:
                        child.kill()  # 终止firda的hook
                        print("end hook !!!")
                    print("uninstall the app!!!")
                    ret = os.system("adb uninstall " + self.app.get_package_name())
                    #if (ret.returncode == 0):
                    #    print("success uninstall ret!!!")
                    break
...
class UtgBasedInputPolicy(InputPolicy):
    ...
    def generate_event(self):   # 这里调用generate_event_based_on_utg()返回event
        """
        generate an event
        @return:
        """
...
class UtgGreedySearchPolicy(UtgBasedInputPolicy):
    ...
    def generate_event_based_on_utg(self):
        ...
        # 当前的activity
        now_activity = self.device.get_current_activity_stack()[0]
        print(now_activity)
        global last_activity
        global last_activity_num
        if now_activity==last_activity:
            if last_activity_num>10:    # 一直在一个界面停留超过10次
                backevent = KeyEvent(name="HOME")
                return backevent
            else:
                last_activity_num += 1
        else:
            ...
            last_activity_num = 0
        if "PackageInstallerActivity" in now_activity: # 软件下载界面
            # 生成一个点击安装按钮的事件
            events = current_state.get_install_input()
            if len(events)!=0:
                return events[0]
        if "com.tencent.mm/" in now_activity:
            # "com.tencent.mm/.plugin.base.stub.UIEntryActivity"
            # "com.tencent.mm/.ui.transmit.SendAppMessageWrapperUI" 朋友圈的activity
            # "com.tencent.mm /.ui.transmit.SelectConversationUI" 分享到好友的activity
            if "com.tencent.mm/.plugin.webview.ui.tools.SDKOAuthUI" in now_activity:    # 如果进入了微信登录界面
                self.logger.info("Go To The Wechat! for Login!!!")
                events = current_state.get_login_input()
                if len(events)!=0:
                    return events[0]
            elif (now_activity == "com.tencent.mm/.plugin.base.stub.WXEntryActivity"): # 如果在验证界面，什么也不做
                backevent = "continue_lsy"
                return backevent
            elif (now_activity == "com.tencent.mm/.ui.transmit.SelectConversationUI"): # 打开了分享到好友的界面
                print("share complete!!!!!")
                print("set end to be true!!!!")
                END_BOT = True
                backevent = KeyEvent(name="BACK")
                return backevent
            elif now_activity == "com.tencent.mm/.plugin.sns.ui.SnsUploadUI"    : # 打开了分享到朋友圈的界面
                print("share to 朋友圈!!!!!")
                sns_events = current_state.get_snsshare_input()
                if len(sns_events)!=0:
                    return sns_events[0]
                backevent = KeyEvent(name="BACK")
                return backevent
                ...
                backevent = KeyEvent(name="BACK")
                return  backevent
        if "com.android.chrome" in now_activity:    # 进入到了google浏览器里，表示要下载某个东西，暂时先不管这种情况
            print("go to the chrome!!!!")
            END_BOT = True
            backevent = KeyEvent(name="HOME")
            return backevent

        if current_state.get_app_activity_depth(self.app) < 0:  # 进入到其它activity时走这个分支
            ...
            if not self.__event_trace.endswith(EVENT_FLAG_START_APP):
                if self.__num_restarts > MAX_NUM_RESTARTS:
                    ...
                    self.__num_restarts += 1
                    self.logger.info("Trying to start the app..."+str(self.__num_restarts))
        ...
        for input_event in possible_events:
            if not self.utg.is_event_explored(event=input_event, state=current_state):
                ...
                global last_id
                try:
                    is_id = input_event.view['resource_id']
                    if is_id == last_id:
                        continue
                    else:
                        last_id = is_id
                    log_str = input_event.event_type +" "+ (is_id if is_id!=None else 'None')
                except:
                    print("get log_str error!!")
                self.logger.info("Trying an unexplored event."+" ["+log_str+"]")
