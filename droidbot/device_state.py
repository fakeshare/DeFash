...
class DeviceState(object):
    ...
    def get_possible_input_old(self):
        """
        ...
        for view_id in enabled_view_ids:    # 优先点击分享的
            choose = False
            if self.__safe_dict_get(self.views[view_id],'visible') == True:
                is_text = self.views[view_id]['text']
                is_id = self.views[view_id]['resource_id']
                if (is_text != None) and (("分享" in is_text) or ("转发" in is_text) or ("登录" in is_text)  or ("微信" in is_text)):  # 看文本
                        choose = True
                if (choose == False) and (is_id != None) and ("share" in is_id):   # 看id
                    choose = True
                if choose == True:
                    if self.__safe_dict_get(self.views[view_id], 'clickable'):
                        ...
                        parent_id = self.__safe_dict_get(self.views[view_id],'parent')
                        if self.__safe_dict_get(self.views[parent_id],'clickable'):
                            possible_events.append(TouchEvent(view=self.views[parent_id]))
                            touch_exclude_view_ids.add(parent_id)
                            touch_exclude_view_ids.union(self.get_all_children(self.views[parent_id]))
        ...
        for view_dict in self.views:
            ...
            if self.__safe_dict_get(view_dict, 'enabled') and \
               ...
               ['android:id/navigationBarBackground',
                ...
                enabled_view_ids.append(view_dict['temp_id'])
        enabled_view_ids.reverse()
        for view_id in enabled_view_ids:    # 优先点击的
            if self.__safe_dict_get(self.views[view_id],'visible') == True:
                is_text = self.views[view_id]['text']
                if (is_text != None) and (("分享" in is_text) or ("转发" in is_text) or ("微信" in is_text) or ("允许" in is_text)):  # 看文本
                    possible_events.append(TouchEvent(view=self.views[view_id]))
                    touch_exclude_view_ids.add(view_id)
                    touch_exclude_view_ids.union(self.get_all_children(self.views[view_id]))
        for view_id in enabled_view_ids:    # 优先点击的
            ...
            if self.__safe_dict_get(self.views[view_id],'visible') == True:
                is_id = self.views[view_id]['resource_id']
                if (is_id != None) and (("share" in is_id) or ("permission_allow_button" in is_id)):   # 看id
                    choose = True
                if choose == True:
                    if self.__safe_dict_get(self.views[view_id], 'clickable'):
                        possible_events.append(TouchEvent(view=self.views[view_id]))
                        ...
                        temp_id = view_id
                        while True:
                            parent_id = self.__safe_dict_get(self.views[temp_id],'parent')
    ...
    def get_snsshare_input(self):
        ...
        for view_dict in self.views:
            ...
            if (self.__safe_dict_get(view_dict, 'enabled') and \
                self.__safe_dict_get(view_dict, 'resource_id') not in \
                ['android:id/navigationBarBackground',
                 'android:id/statusBarBackground']) and self.__safe_dict_get(view_dict, 'visible') == True:
                enabled_view_ids.append(view_dict['temp_id'])
        enabled_view_ids.reverse()
        for view_id in enabled_view_ids:    # 优先点击的
            if self.__safe_dict_get(self.views[view_id],'visible') == True:
    ...
    def get_install_input(self):
        ...
        for view_id in enabled_view_ids:  # 优先点击的
            is_id = self.views[view_id]['resource_id']
            if is_id=='com.android.packageinstaller:id/ok_button':  # 看id
                possible_events.append(TouchEvent(view=self.views[view_id]))
                touch_exclude_view_ids.add(view_id)
                touch_exclude_view_ids.union(self.get_all_children(self.views[view_id]))
        self.possible_events = possible_events
    ...
    def get_login_input(self):
        ...
        for view_id in enabled_view_ids:  # 优先点击的
            is_id = self.views[view_id]['resource_id']
            if is_id=='com.tencent.mm:id/d5p':  # 看id
                possible_events.append(TouchEvent(view=self.views[view_id]))
                touch_exclude_view_ids.add(view_id)
                touch_exclude_view_ids.union(self.get_all_children(self.views[view_id]))
        self.possible_events = possible_events
